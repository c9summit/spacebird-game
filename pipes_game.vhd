...
ARCHITECTURE behaviour OF pipes_game IS
    ...
    -- Speed: 1 unit per frame in training, increases in normal
    CONSTANT TRAIN_SPEED : INTEGER := 2;
    ...
    -- Score counter to drive speed increases in normal mode
    SIGNAL pass_count : INTEGER RANGE 0 TO 255 := 0;
    SIGNAL pipe_speed : INTEGER RANGE 1 TO 6   := TRAIN_SPEED;
    SIGNAL passed     : STD_LOGIC := '0'; -- pipe just passed screen left

BEGIN

    -- Speed selection: training = constant, normal = increases with score
    ...
    BEGIN
        ...
        ELSE
            IF pass_count < 5 THEN
                pipe_speed <= 2;
            ELSIF pass_count < 10 THEN
                pipe_speed <= 3;
            ELSIF pass_count < 15 THEN
                pipe_speed <= 4;
            ELSIF pass_count < 20 THEN
                pipe_speed <= 5;
            ELSE
        ...
        IF rising_edge(clk) THEN
            ...
            ELSIF scroll_en = '1' AND vsync_d = '0' AND vert_sync = '1' THEN
                ...
                IF pipe_x <= -PIPE_W THEN
                    -- Pipe off left edge: reset to right and pick new gap
                    pipe_x  <= SCREEN_W;
                    passed  <= '0';
                ...
                ELSE
                    ...
                    -- Count pipe as passed when it crosses x=0
    ...
    -- Draw logic: combinational
    PROCESS(pixel_row, pixel_column, pipe_x, gap_top)
        VARIABLE pr         : INTEGER;
        VARIABLE pc         : INTEGER;
        VARIABLE gap_bottom : INTEGER;
        VARIABLE in_pipe    : STD_LOGIC;
        VARIABLE in_border     : STD_LOGIC;
    BEGIN
        ...
        IF pc >= pipe_x AND pc < pipe_x + PIPE_W THEN
            IF pr < gap_top OR pr >= gap_bottom THEN
                in_pipe := '1';
                -- Border highlight: top/bottom edges of each pipe section and sides
                IF pr = gap_top - 1 OR pr = gap_bottom OR
                   pc = pipe_x OR pc = pipe_x + PIPE_W - 1 THEN
                    in_border := '1';
                END IF;
            END IF;
        END IF;
        ...
        IF in_pipe = '1' THEN
            IF in_border = '1' THEN
                -- Bright cyan border for space feel
                pipe_red   <= "0000";
                pipe_green <= "1111";
                pipe_blue  <= "1111";
            ELSE
                -- Dark blue/grey asteroid barrier body
                pipe_red   <= "0010";
                pipe_green <= "0011";
                pipe_blue  <= "0101";
            END IF;
    ...
    -- Expose pipe position for collision detection
