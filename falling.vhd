...
ARCHITECTURE behaviour OF falling IS
    ...
    -- Full-width relative position signals
...
BEGIN
    ...
    -- Bounding box check
    ball_on <= '1' WHEN (
        pixel_column >= ball_x_pos AND
        pixel_column <  ball_x_pos + SPRITE_W AND
        pixel_row    >= ball_y_pos AND
        pixel_row    <  ball_y_pos + SPRITE_H)
        ELSE '0';

    -- Full subtraction for correct sprite-relative address
    rel_col <= pixel_column - ball_x_pos;
    rel_row <= pixel_row    - ball_y_pos;

    -- Address = row(3:0) & col(4:0), 9 bits total
